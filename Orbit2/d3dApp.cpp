...
void D3DApp::initApp()
{
	...
	wcscpy_s(fontDesc.FaceName, L"Neuropol");

	D3DX10CreateFontIndirect(md3dDevice, &fontDesc, &mFontNormal);

	fontDesc.Height          = 12;
	D3DX10CreateFontIndirect(md3dDevice, &fontDesc, &mFontSmall);
	fontDesc.Height          = 8;
	D3DX10CreateFontIndirect(md3dDevice, &fontDesc, &mFontSmallest);
}
...
void D3DApp::updateScene(float dt)
{
	...
	float updatetime = 0.25f;//in seconds
	if( (mTimer.getGameTime() - t_base) >= updatetime )
	{
		float fps = (float)frameCnt*(1/updatetime); // fps = frameCnt / 1
		float mspf = (1000.0f*updatetime) / fps;
...
void D3DApp::initMainWindow()
{
	...
	//LPCWSTR FileName = LPCWSTR("arrow.ani");
	CursorDefault = LoadCursorFromFile(LPCWSTR("arrow.ani"));
	
	if (CursorDefault == NULL)
		wc.hCursor       = LoadCursor(0, IDC_HAND);
	else
		wc.hCursor       = CursorDefault;
