...
VS_Glow_OUT VGlow(VS_Glow_IN vIn)
{
	VS_Glow_OUT vOut;

	// Make glow slightly larger then object
	vIn.posL.xyz = 1.1 * vIn.posL.xyz;
...
float4 PGlow(VS_Glow_OUT pIn) : SV_Target
{
	return gCubeMap.Sample(gTriLinearSam, float3(pIn.texC, 0));
}
technique10 CubeMapTech
{
	...
	//pass Glow
    //{
    //    SetVertexShader( CompileShader( vs_4_0, VGlow() ) );
    //    SetGeometryShader( NULL );
