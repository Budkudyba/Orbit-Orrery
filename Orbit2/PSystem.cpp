...
void PSystem::setSize(float s)
{
	mSize = s;
}
void PSystem::setRate(float r)
{
	mRate = r;
}
void PSystem::setKill(float k)
{
	mKill = k;
}

void PSystem::setColor(D3DXCOLOR c)
{
	mColor = c;
}
				   ...
				   UINT maxParticles)
{
	...
	mfxSizeVar  = FX->GetVariableByName("gSize")->AsScalar();
	mfxRateVar  = FX->GetVariableByName("gRate")->AsScalar();
	mfxKillVar  = FX->GetVariableByName("gKill")->AsScalar();
	mfxColorVar   = FX->GetVariableByName("gColor")->AsVector();
...
void PSystem::draw()
{
	...
	mfxSizeVar->SetFloat(mSize);
	mfxRateVar->SetFloat(mRate);
	mfxKillVar->SetFloat(mKill);

	mfxColorVar->SetFloatVector(mColor);
	//
	// Set IA stage.
	//
