...
void TreeSprites::init(ID3D10Device* device, const D3DXVECTOR3 centers[], const D3DXVECTOR2 sizes[], UINT numTrees)
{
	md3dDevice = device;
...
void TreeSprites::buildVB(const D3DXVECTOR3 centers[], const D3DXVECTOR2 sizes[])
{
	TreeVertex* v = new TreeVertex[mNumTrees];
	//float sunSize = 1.392e6*.000039;

	for(UINT i = 0; i < mNumTrees; ++i)
	{
		v[i].centerW  = centers[i];
		v[i].sizeW    = sizes[i];//D3DXVECTOR2(2.0f, 2.0f);
		//v[i].sizeW    = D3DXVECTOR2(sunSize, sunSize);//24,24 //needs to be modifiable
	}
...
void TreeSprites::buildShaderResourceView()
{
	...
	//
	const int numtexture = 1;

	std::wstring filenames[numtexture] = 
	{
		//L"tree0.dds",
		//L"tree1.dds",
		//L"tree2.dds",
	...
	ID3D10Texture2D* srcTex[numtexture];
	...
	texArrayDesc.ArraySize          = numtexture;
	...
	viewDesc.Texture2DArray.ArraySize = numtexture;
...
void TreeSprites::move(const D3DXVECTOR3 centers[])
{
