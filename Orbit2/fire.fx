...
cbuffer cbFixed
{
	...
	// rate to emit particles per second
	float pRate = 0.08f;//.005f;
	// Size to make particles
	float pSize = 0.25f;
	// time till kill particle
	float pKill = 5.0f;
...
{	
	...
	{	
		...
		if( gIn[0].age > gRate )
		{
			...
			p.initialVelW = 0.0f;//4.0f*vRandom;
			p.sizeW       = float2(gSize, gSize);
			p.age         = 0.0f;
			p.type        = PT_FLARE;
...
VS_OUT DrawVS(Particle vIn)
{
	...
	float opacity = smoothstep(0.0f, 1.0f, t/1.0f);//fade in
	if (t > gKill*0.9)
		opacity = 1 - smoothstep(0.0f, 1.0f, t-gKill*0.9/1.0); //fade out
	vOut.color = float4(gColor.rbg, opacity);
