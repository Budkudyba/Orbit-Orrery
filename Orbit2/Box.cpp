...
Box::Box()
: mNumVertices(0), mNumFaces(0), md3dDevice(0), mVB(0), mIB(0), mNumSubsets(0)
{
}
...
void Box::init(ID3D10Device* device, float scale)
{
	...
	mTech         = fx::TexFX->GetTechniqueByName("TexTech");
	mfxWVPVar     = fx::TexFX->GetVariableByName("gWVP")->AsMatrix();
