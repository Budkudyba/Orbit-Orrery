...
public:
	...
	void init(ID3D10Device* device, const D3DXVECTOR3 centers[], const D3DXVECTOR2 sizes[], UINT numTrees);
	void draw(const Light& L, const D3DXVECTOR3& eyePosW, const D3DXMATRIX& viewProj);
	void move(const D3DXVECTOR3 centers[]);

private:
	void buildVB(const D3DXVECTOR3 centers[], const D3DXVECTOR2 sizes[]);
	void buildFX();
